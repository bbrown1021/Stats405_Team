...
author: "Britney Brown, Harrison DiStefano, Lisa Kaunitz, Tianyang Liu, Jeremy Weidner"
date: "05/02/2021"
...
```{r SET UP, echo=FALSE, message=FALSE, eval=TRUE, results='hide'}
...
We will be using two tables from the given database: "nhl_gameDate_1" and "nhl_playersDate_1"

The first table provides information such as the game date, the visiting and home teams, and the names of the officials. The second table provides infromation on a player's first and last name, the team they played for, and the total goals scored in a game. By joining these two tables together and filtering for players that scored more than `r xpntsCO` goals, we will be able to provide an answer to the above question of interest. 

```{r runQuery1, echo=FALSE, eval=TRUE, results='hide'}
qstr <- "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='nhl_gameDate_1'"
xx <- dbGetQuery(con, qstr)
#xx

qstr <- "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='nhl_playersDate_1'"
xx <- dbGetQuery(con, qstr)
#xx
...
"JOIN nhl_playersDate_1 ON (nhl_gameDate_1.date=nhl_playersDate_1.date AND nhl_gameDate_1.VT=nhl_playersDate_1.team)  ",
" OR (nhl_gameDate_1.date=nhl_playersDate_1.date AND nhl_gameDate_1.HT=nhl_playersDate_1.team)  ",
"WHERE nhl_playersDate_1.goals > ", xpntsCO
)
qstr

# output query
df_out <- dbGetQuery(con, qstr)

class(df_out) # data frame
